  ...
  import { ImageFolder } from '../../shared/types/upload.types';
  ...
  export const uploadAvatar = async (req: Request, res: Response) => {
    try {
      ...
      if (user.avatar) {
        const oldAvatar = user.fileUploads.find(
          (upload: any) =>
            upload.tags?.includes('avatar') &&
            upload.tags?.includes(`user-${userId}`)
        );

        if (oldAvatar?.cloudinaryId) {
          await uploadService.deleteImage(oldAvatar.cloudinaryId);
        }
      }
      ...
      const optimizedAvatar = uploadService.generateOptimizedUrl(
        uploadResult.url,
        500,
        500
      );
      const thumbnailAvatar = uploadService.generateOptimizedUrl(
        uploadResult.url,
        150,
        150
      );
      ...
      // Save upload record
      // await uploadService.saveUploadRecord(
      //   {
      //     filename: uploadResult.name,
      //     originalName: file.originalname,
      //     mimeType: file.mimetype,
      //     size: file.size,
      //     // Store the same URL that we keep on the user model
      //     url: optimizedAvatar,
      ...
      //   Number(userId)
      // );
      console.log('optimizedAvatar', optimizedAvatar);
  ...
  export const deleteAvatar = async (req: Request, res: Response) => {
    try {
      ...
      const avatarUpload = user.fileUploads.find((upload: any) =>
        upload.tags?.includes('avatar') &&
        upload.tags?.includes(`user-${userId}`)
      );

      if (avatarUpload?.cloudinaryId) {
        await uploadService.deleteImage(avatarUpload.cloudinaryId);
      }
