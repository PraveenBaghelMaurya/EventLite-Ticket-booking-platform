...
export const createEvent = async (req: Request, res: Response) => {
    try {
        ...
        if (!event.title || !event.description || !event.venue || !event.street ||
            !event.city || !event.state || !event.country || !event.postalCode ||
            !event.startDate || !event.endDate || !event.price || !event.capacity ||
            !event.availableTickets || !event.status || event.isFeatured === undefined ||
            event.isPublic === undefined) {
            return ApiResponse.error(res, { message: "All event fields are required" })
        }
...
export const filterEvent = async (req: Request, res: Response) => {
    try {
        ...
        orderBy[validSortFields.includes(sortBy as string) ? sortBy as string : 'startDate'] =
            validSortOrders.includes(sortOrder as string) ? sortOrder as string : 'asc';
        ...
        if (searchQuery) {
            filter.AND.push({
                OR: [
                    ...
                    { shortDescription: { contains: searchQuery as string, mode: 'insensitive' } }
                ]
            });
        }
        ...
        if (timeRange) {
            ...
            switch (timeRange) {
                ...
                case 'next-3-months':
                    ...
                    startDateFilter.lte = new Date(now.getFullYear(), now.getMonth() + 3, now.getDate());
                    break;
                case 'upcoming':
                    startDateFilter.gte = now;
                    break;
